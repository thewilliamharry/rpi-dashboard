# Requirements Archive: v1.0 Beacon
...
**Final status at v1.0 close (2026-09-07): 45 of 46 v1 requirements Complete.**

| Group | IDs | Outcome |
|-------|-----|---------|
| Foundation Integrity | FND-01..FND-07 | ✓ Validated — Phase 1, verified 8/8 |
| Telemetry and Retention | TEL-01..TEL-06 | ✓ Validated — Phase 2, verified 4/4 |
| Planned Maintenance | MNT-01..MNT-04 | ✓ Validated — Phase 03.1 (INSERTED), verified 9/9 |
| Advanced Diagnosis | DIA-01..DIA-08 | ✓ Validated — Phase 3, verified 5/5 (round 9) |
| Advanced Diagnosis | DIA-09 | ✓ Validated — Phase 7, verified 5/5. **Wording adjusted**, not scope-reduced: amended 2026-09-07 by D-07-10 so the shared `/api/telemetry/history` and `/api/events/history` stay ungated by the toggle, because they are Phase 2 and Phase 4 deliverables owned by TEL-05 and HIS-01..06 rather than advanced diagnostics' own routes. |
| Historical Investigation | HIS-01..HIS-06 | ✓ Validated — Phase 4, verified 6/6. HIS-04 was found broken by `04-VERIFICATION.md` and restored by gap-closure plans 04-09/04-10/04-11. |
| Experience and Themes | UX-01..UX-07 | ✓ Validated — Phase 5, verified 5/5 |
| Operational Resilience | OPS-01..OPS-06 | ✓ Validated — Phase 6, verified 5/5 (1 by override) |
| Operational Resilience | OPS-07 | ⚠️ **Accepted with deviation — never promoted to Complete.** See below. |

**OPS-07 outcome in full.** Cadence, resources, recovery and sampling continuity all PASS on
Pi-class hardware. `/api/services` misses its 500 ms p95 budget on three independent runs
(635.6 / 679.3 / 662.3 ms) under a closed-loop harness measured at ~34.5× the deployment's real
0.067 req/s per-route rate; at the actual rate the route measures 77.1 ms. Accepted on usage
grounds by operator decision of 2026-09-06, recorded in `06-ACCEPTANCE-C3-RUN3.md`. No budget,
success criterion, assertion or harness default was amended to manufacture a pass —
`PROH-OPS-07-01` and `PROH-OPS-07-10` were verified intact, and `ROUTE_BUDGETS_MS` is
byte-identical to its introduction in `807776a`. All three failing runs stand unsuperseded.
`PROH-OPS-07-08` reserves promotion to a future independent round. Revisit only if the real
request rate rises or services are added — `D-DEBT-06-27`.

**Requirements changed during the milestone:** one. DIA-09's wording, amended by D-07-10 on
ownership grounds after Phase 7's verification round; the original text, the amendment, its
counter-argument and its residual are all recorded in `07-DECISIONS.md`. No requirement was
dropped, and none was weakened to fit what was built.

**Coverage note:** the `## Traceability` coverage block above reads "45 total", which was stale
by one — the correct v1 count is 46 (45 Complete + OPS-07). Corrected here at archive time
rather than edited in the archived body, per this project's convention that a recorded document
is annotated, not rewritten.

**Carried forward to v2 (candidates, not commitments):** Safe Remote Actions, ACT-01..ACT-04.
