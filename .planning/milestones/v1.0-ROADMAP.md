...
**Phases:** 1-7 (8 phases, including the inserted 03.1)
**Total Plans:** 129 exist · 127 executed
**Closeout type:** `override_closeout`

## Overview

Beacon becomes a dependable, self-contained Raspberry Pi operations dashboard by first
protecting the working product and its data, then establishing bounded and truthful telemetry,
adding an advanced diagnosis and investigation workspace, and finally proving a theme-parity
experience remains responsive while low-priority discovery and previews run. Phase 03.1 was
inserted mid-milestone for planned-maintenance recognition, and Phase 7 added the ability to
run with advanced diagnostics switched off. The milestone retained Flask, SQLite WAL, Docker
Compose and the two-process deployment throughout; it introduced no hosted monitoring, broker,
ORM or frontend build stack.
...
- Phase 03.1: Planned Maintenance Recognition (INSERTED after Phase 3) — confirmed recurring
  restart windows had to be designed before further surfaces consumed events, or suppression
  rules would have been retrofitted onto consumers that already assumed every down event was
  unexpected. Reopened 2026-08-23 after human UAT found migration 9 could not apply to any
  existing deployment (G-03.1-1, G-03.1-2, G-03.1-2b); all resolved and the migration support
  floor fixed.

**Key Decisions:**

- Build foundation and advanced analytics in one milestone (Rationale: analytics needs
  trustworthy collection, persistence and UI boundaries first) — ✓ Good
- Keep the advanced workspace strictly read-only, GET-only, with no operation endpoint
  (Rationale: reinforces the deferred-remote-actions boundary at the surface an operator uses
  during a live incident) — ✓ Good; 46 blocking threats verified, `threats_open: 0`
- Derive every operator-facing label from the durable row it describes (Rationale: the
  milestone's recurring defect class was truthful labelling, not missing features) — ✓ Good;
  the highest-value convention v1.0 produced
- Never erase a recorded finding, even when superseded (Rationale: a monitoring product that
  edits its own history teaches its authors the wrong habit) — ✓ Good
- Fix OPS-07's gate at concurrency 3 (Rationale: the ceiling a single operator generates;
  real per-route rate is 0.067 req/s) — ✓ Good, and load-bearing because it was chosen before
  any run failed
- Accept OPS-07 with deviation rather than promote or weaken it — ⚠️ Revisit per `D-DEBT-06-27`
- Amend DIA-09's wording (D-07-10) rather than gate the shared history APIs behind the advanced
  toggle (Rationale: `/api/telemetry/history` and `/api/events/history` are owned by TEL-05 and
  HIS-01..06) — ✓ Good; an ownership correction, not a scope reduction
- Let `api_advanced_current` skip the process-global `_db_lock` (AR-03-01) — ✓ Good; revisited
  in Phase 6 alongside the WAL decision and left standing

**Issues Resolved:**

- Import-time lifecycle side effects, cross-process SQLite coordination, and duplicate execution
  of shared scheduled work
- Migration 9's inability to apply to any existing deployment (found by human UAT, not by tests)
- The `_OFFLINE_INTERVALS_BULK_ROW_LIMIT` row cap silently bounding the uptime path — a service
  with 20.833% true uptime reported as 100.0%; fixed in `bcad398`, query now deliberately
  unbounded with a comment naming both consumers' differing correctness requirements
- A `null` latency rendering as `0 ms` and sorting as the fastest service
- A failed unfiltered-baseline read rendering the filtered count as the total (`"N of N"` for a
  genuinely unknown total)
- Background jobs recording a `succeeded` or `failed` outcome they had not established, including
  an empty durable queue producing two fabricated failure cards
- Environment leakage from `load_app`'s `extra_env` causing a deterministic cross-module
  regression (the concrete instance fixed; the helper itself left as debt)

**Issues Deferred:**

- The concurrency-1 HTTP control run on `a7c3ef1` (Phase 6's residual `human_needed` item)
- `PROH-DIA-09-01`'s per-service compose-override escape (Phase 7's open gap 2, found by
  mutation M8)
- Phase 5's three CONTEXT questions, superseded by the phase shipping
- Seventeen carried-forward `03-REVIEW.md` round-1 findings

**Technical Debt Incurred:**

- `D-DEBT-06-27` — selective non-uniform inflation under concurrency-3 load, consistent with
  lock contention, not yet attributed. The named revisit trigger for OPS-07.
- `D-DEBT-06-16` — non-`api_services` sites holding non-database work under `_db_lock`,
  found by the 28-site audit and deliberately left untouched.
- `D-DEBT-06-17` — `/api/advanced/current`'s excluded batching candidate and topology option T-A.
- `D-DEBT-06-11` — the offline-interval reconstruction path's row cap silently bounds
  `maintenance_attributed_seconds`. Separable from the closed uptime defect, same defect class.
- `D-DEBT-07-01` — the Pi acceptance harness records `elapsed_ms` but never `status_code`, so a
  404 reads as a fast, budget-clearing success. Owned by the next OPS-07 round.
- `dashboard/beacon/diagnosis.py:448` — a hardcoded three-step `resolution_policy` literal
  against `telemetry.py`'s ten-step `RESOLUTION_LADDER_SECONDS`. Zero production consumers, but
  served in `/api/advanced/current` and able to drift silently.
- `tests/helpers.py::load_app` — writes `extra_env` into `os.environ` and never restores it.
- Wave grouping compares `files_modified` paths only, so it cannot see a semantic dependency
  where one plan changes a constant another plan's tests assert on (hit at the Phase 5 wave-4
  merge). Candidate fix: have plans declare `constants_changed` alongside `files_modified`.
