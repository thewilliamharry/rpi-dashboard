...
result: pass
evidence: |
  Confirmed by the operator on real hardware 2026-08-20 ~23:05 local, after
  commit 07ac39c extended the affordance to every remaining control.
  The tints are 8-12% alpha and the light-mode border shift is subtle by design —
  this item existed because whether they are actually visible at the Pi's own
  display brightness and viewing angle is a judgement call outside
  `getComputedStyle`'s reach. The computed deltas themselves were already pinned by
  `test_every_interactive_control_reads_as_interactive_in_both_themes`; this
  confirms they are perceptible in practice, not merely applied.
...
result: pass
evidence: |
  Re-confirmed by the operator on real hardware 2026-08-20 ~23:08 local, after
  round 9's change. Originally recorded PASS 2026-08-20 ~13:00 (see this file's
  git history at the round-8 UAT). Round 9's change is stylesheet-only and touches
  no logic file this test depends on — `git diff 2633b9e..07ac39c --
  dashboard/beacon/diagnosis.py` is empty — so this re-confirms the prior result
...
result: pass
evidence: |
  Re-confirmed by the operator on real hardware 2026-08-20 ~23:11 local, after
  round 9's change. Originally recorded PASS 2026-08-20 13:03 (see this file's git
  history at the round-8 UAT). Round 9's change is stylesheet-only, so this
  re-confirms the prior result rather than establishing a new one.
  NOTE: this test is the NEGATIVE half only — absence of a job card cannot by
  itself distinguish "jobs healthy" from "promotion broken". Test 5 is the
...
result: pass
evidence: |
  Re-confirmed by the operator on real hardware 2026-08-20 ~23:14 local, after
  round 9's change. Originally recorded PASS 2026-08-20 21:43 (see this file's git
  history at the round-8 UAT), with a noted caveat that the render itself was not
  photographed within the transient failure window — tracked as a Deferred
  Follow-Up, not a gap. Round 9's change is stylesheet-only, so this re-confirms
  the prior result rather than establishing a new one.
  This is the discriminating POSITIVE counterpart to test 4: it proves job
  promotion actually fires, not merely that it stays quiet on an idle system.
...
- gap_id: G-03-4
  ...
  status: resolved
  resolved_by: "03-23-PLAN.md (commits 2b12c4b, 07ac39c)"
  resolved_at: 2026-08-20T22:41:00Z
  evidence: |
    ...
    zero defects in this diff. Both perception halves confirmed by the operator on
    real hardware: `Refresh now` as the primary action (test 1) and hover feedback
    across every remaining control (test 2).
