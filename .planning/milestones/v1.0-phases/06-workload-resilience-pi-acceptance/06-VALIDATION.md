...
| **Full suite runtime** | **Measured 2026-09-01 (06-01 Task 4):** 222.82s wall-clock (`time uv run --project dashboard python -m pytest -q`; 780 passed, 552 subtests passed), measured against a green suite. Eleven tasks across this phase assert on an unscoped full-suite run; this measured number is the contract they are budgeted against (see the reconciliation applied in § Sampling Rate below). |
...
- **Max feedback latency (full suite):** 240 seconds, **measured** (06-01 Task 4, 2026-09-01): the observed 222.82s full-suite runtime exceeded the provisional 120-second budget, so per the reconciliation rule this line is replaced with the measured value rounded up to the next 30 seconds (222.82s -> 240s), annotated as measured rather than assumed. The quick-command budget above is not weakened by this change — the two commands carry separate budgets because they answer different questions.
