...
| 19 | `api_services` | 2821 | combined | read | none — narrowed in `06-20`. Was: `_uptime_summary`, `beacon_maintenance.coverage`, `beacon_maintenance.attributed_downtime_seconds`, `beacon_repositories.offline_intervals_from_points_by_port`, and the per-service `result` dict construction, measured **25.0% of this route's held region** under concurrency-8 load (`06-LOCK-DIAGNOSTIC.md` §4, `beacon-lockdiag-c8.json`) before the narrowing. All four now execute after the block closes, over rows materialized into plain dicts inside it (`D-DEBT-06-01`, `PROH-OPS-04-06`). | **yes (landed)** |
