...
PROFILE_PHASES = {
    ...
    'maintenance_coverage': [
        ...
        ('beacon/maintenance.py', 'parse_weekdays'),
        ('beacon/maintenance.py', 'resolve_timezone'),
        ('beacon/maintenance.py', 'Window.__post_init__'),
    ],
    'covering_boundaries': [
        ('beacon/maintenance.py', '_covering_boundaries'),
        ('beacon/maintenance.py', '_anchor_step_budget'),
    ],
    'json_serialization': [
        ('*', 'jsonify'),
...
def _resolve_attr_chain(module, dotted_name):
    """Resolve a possibly-dotted attribute path (e.g. 'Window.__post_init__' for a
    dataclass method) against a live module.
    """
    obj = module
...
def _resolve_range_rules(appmod):
    ...
    for bucket, pairs in PROFILE_PHASES.items():
        for filename_suffix, function_name in pairs:
            ...
            if module is None:
                continue
            func = _resolve_attr_chain(module, function_name)
...
def _classify_key(key, range_rules):
    ...
    bucket by direct name/range match, or 'other' if nothing declared matches it.

    This is the DIRECT classification only. It intentionally does not resolve
    the many builtin/leaf entries (max, min, sorted, datetime.replace, an
    autogenerated dataclass __init__, ...) that a matched function's body
    calls into but that cProfile always reports as their own independent
    pstats entry, regardless of caller -- see
    ``_classify_with_call_graph`` for the caller-aware redistribution pass
    that attributes those leaves back to the bucket that invoked them.
    """
    ...
    for bucket, pairs in PROFILE_PHASES.items():
        for filename_suffix, function_name in pairs:
            match_name = function_name.rsplit('.', 1)[-1]
            if filename_suffix == '~':
                if filename == '~' and match_name in funcname:
                    return bucket
            elif filename_suffix == '*':
                if funcname == match_name:
                    return bucket
            else:
                if filename.endswith(filename_suffix) and funcname == match_name:
                    return bucket
    return 'other'


def _classify_with_call_graph(stats_dict, range_rules):
    """Classify every pstats entry into a bucket, redistributing an
    unmatched leaf's self time to its callers' buckets.

    cProfile gives a called C builtin (max, min, sorted, datetime.replace,
    ...) or an undeclared small helper its own independent pstats entry with
    its own self ("tot") time, entirely separate from whichever matched
    function called it. Leaving that leaf time in 'other' would
    systematically undercount every bucket that relies on ordinary Python
    builtins to do its work -- which is nearly all of them. Instead, for
    every entry this module's declared PROFILE_PHASES pairs did not directly
    match, this walks pstats' own recorded ``callers`` mapping (which
    entries called this one, and how many times) and splits this entry's
    self time across its callers' buckets, weighted by each caller's share
    of the calls into it. An entry whose caller itself cannot be resolved to
    a declared bucket keeps that share in 'other' rather than guessing.
    Returns (bucket_tottime, bucket_calls), both dicts of bucket name to
    float (bucket_calls is rounded to the nearest whole call by the caller).
    """
    direct_bucket = {key: _classify_key(key, range_rules) for key in stats_dict}
    bucket_tottime = defaultdict(float)
    bucket_calls = defaultdict(float)
    for key, value in stats_dict.items():
        _primitive_calls, num_calls, tottime, _cumtime, callers = value
        bucket = direct_bucket[key]
        if bucket != 'other' or not callers:
            bucket_tottime[bucket] += tottime
            bucket_calls[bucket] += num_calls
            continue
        total_caller_calls = sum(caller_value[1] for caller_value in callers.values())
        if total_caller_calls <= 0:
            bucket_tottime['other'] += tottime
            bucket_calls['other'] += num_calls
            continue
        for caller_key, caller_value in callers.items():
            caller_calls = caller_value[1]
            caller_bucket = direct_bucket.get(caller_key, 'other')
            share = caller_calls / total_caller_calls
            bucket_tottime[caller_bucket] += tottime * share
            bucket_calls[caller_bucket] += num_calls * share
    return bucket_tottime, bucket_calls
...
def profile_services_route(appmod, *, repeats=5):
    ...
    bucket_tottime, bucket_calls = _classify_with_call_graph(stats.stats, range_rules)
    total_tottime = sum(bucket_tottime.values())
    ...
    for bucket, tottime in bucket_tottime.items():
        ...
        phases[bucket] = {
            ...
            'calls': int(round(bucket_calls[bucket])),
