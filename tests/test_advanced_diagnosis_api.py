...
class AdvancedCurrentCostTests(_MaintenanceDiagnosisFixture):
    ...
    # REMOVED 2026-09-03 — test_occurrence_walks_do_not_scale_with_port_count.
    #
    # It asserted that get_current_diagnosis' occurrence-walk count stays flat
    # as port count grows, which was true only while 06-22's request-scoped
    # memo (T-C) was threaded through. That memo was reverted after round 5's
    # hardware measurement: it did not recover 06-20's regression, it deepened
    # it — /api/advanced/current's non-lock off-CPU went 550,980ms (round 4)
    # → 1,007,408ms (post-narrowing) → 1,090,992ms (post-memo). Full evidence
    # in 06-LOCK-DIAGNOSTIC-R5B.md.
    #
    # Deleted rather than inverted: asserting walks DO scale with port count
    # pins an inefficiency as though it were a contract, and would have to be
    # deleted again by whoever reintroduces caching. The payload-equivalence
    # golden (test_advanced_current_payload_matches_the_pre_remedy_golden)
    # survives and still guards the response bytes, which is the property that
    # matters regardless of how the value is computed.
    #
    # If a future round reintroduces memoization here — likely, since the
    # rollup work makes the same class of change — restore this guard from
    # commit afff388.
